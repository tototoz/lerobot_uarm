...
from .config_uarm1 import uarm1Config
import numpy as np

logger = logging.getLogger(__name__)

class uarm1(Teleoperator, Node):
    config_class = uarm1Config
    name = "uarm1"

    def __init__(self, config: uarm1Config):
        # 检查 ROS2 是否已经初始化，如果没有则初始化
        if not rclpy.ok():
            rclpy.init()
        
        # 初始化 ROS2 节点
        Node.__init__(self, "uarm1_node")  # 初始化 ROS2 节点
